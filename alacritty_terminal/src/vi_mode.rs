...
impl ViModeCursor {
    ...
    pub fn scroll<T: EventListener>(mut self, term: &Term<T>, lines: isize) -> Self {
        ...
        let col = first_occupied_in_line(term, target_line as usize)
            .unwrap_or_default()
            .col;
    ...
    P: Into<Point<usize>>,
{
    ...
    if cell.flags.contains(Flags::WIDE_CHAR) && !left {
        ...
        && term.grid()[point.line][point.col - 1]
            .flags
...
fn is_wrap<T>(term: &Term<T>, point: Point<usize>) -> bool {
    term.grid()[point.line][point.col]
        .flags
...
mod tests {
    ...
    fn scroll_semantic() {
        let mut term = term();
        term.grid_mut()
            .scroll_up(&(Line(0)..Line(20)), Line(5), &Default::default());
