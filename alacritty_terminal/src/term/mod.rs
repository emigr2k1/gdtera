...
impl<T> Search for Term<T> {
    ...
    fn line_search_left(&self, mut point: Point<usize>) -> Point<usize> {
        while point.line + 1 < self.grid.len()
            && self.grid[point.line + 1][self.grid.num_cols() - 1]
                .flags
    ...
    fn line_search_right(&self, mut point: Point<usize>) -> Point<usize> {
        while self.grid[point.line][self.grid.num_cols() - 1]
            .flags
            .contains(Flags::WRAPLINE)
        {
            point.line -= 1;
...
impl<T> Term<T> {
        ...
        T: EventListener,
    {
        ...
        if self.dynamic_title {
            ...
            self.event_proxy
                .send_event(Event::Title(self.default_title.clone()));
        }

        if self.alt {
            self.alt_grid
                .update_history(config.scrolling.history() as usize);
        } else {
            self.grid
                .update_history(config.scrolling.history() as usize);
        }
    ...
    pub fn resize(&mut self, size: &SizeInfo) {
        ...
        let alt_cursor_point = if is_alt {
    ...
    fn scroll_down_relative(&mut self, origin: Line, mut lines: Line) {
        ...
        self.grid
            .scroll_down(&(origin..self.scroll_region.end), lines, &template);
    }
    ...
    fn scroll_up_relative(&mut self, origin: Line, lines: Line) {
        ...
        self.grid
            .scroll_up(&(origin..self.scroll_region.end), lines, &template);
    }
    ...
    pub fn toggle_vi_mode(&mut self) {
        ...
        if self.mode.contains(TermMode::VI) {
            let line = min(
                self.cursor.point.line + self.grid.display_offset(),
                self.lines() - 1,
            );
    ...
    fn renderable_cursor<C>(&self, config: &Config<C>) -> RenderableCursor {
        ...
        let is_wide = if cell.flags.contains(Flags::WIDE_CHAR_SPACER)
            && self.grid[buffer_point.line][buffer_point.col - 1]
                .flags
...
impl<T: EventListener> Handler for Term<T> {
    ...
    fn dynamic_color_sequence<W: io::Write>(
        ...
        trace!(
            "Writing escape sequence for dynamic color code {}: color[{}]",
            code,
            index
        );
